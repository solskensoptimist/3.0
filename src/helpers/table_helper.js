    ...
    getFleetColumns: (historic) => {
        if (historic) {
            return [
                ...
                {id: 'reg_number', numeric: false, label: tc.regNumber},
                {id: 'sale_new', numeric: false, label: tc.bought},
...
const getFleetRowsHistoric = (rows) => {
    ...
    return rows.map((row) => {
        const obj = {};
        columns.forEach((column) => {
            if (column.id === 'sale_new') {
                obj[column.id] = (row.sale_new === 0) ? tc.used : tc.new
            } else if (column.id === 'holdingPeriod') {
